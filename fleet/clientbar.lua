...
clientbar.enable_tooltip = true

local all_clientbars = setmetatable({}, { __mode = 'k' })
...
local function new(c, args)
    ...
    if not bar then
        ...
        ret = drawable(d, context, "fleet.clientbar")
        ret:_inform_visible(true)

        local function update_colors()
            local args_ = bar.args
...
return setmetatable(clientbar, { __call = function(_, ...) return new(...) end})
